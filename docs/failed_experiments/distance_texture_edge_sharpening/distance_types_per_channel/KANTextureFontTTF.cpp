...
#ifndef USE4CHANNEL_FONT_TEXTURE
#define DEFAULT_PIXEL_VALUE pixel_type(0,0)
#else
...
namespace
{
  ...
  pixel_value_from_distance(float dist, bool outside)
  {
    ...
    dist+=1.0f;
    dist/=2.0f;
    return static_cast<GLubyte>(255.0f*dist);
  }
  **/
  ...
  public:
    ...
    {
      m_point_value=v;
      m_from_grids_value=vec2(v, v);
    }

    
    void
    update_point_value(float v)
    {
      m_point_value=std::min(v, m_point_value);
    }

    void
    update_fixed_grid_value(float v, enum coordinate_type tp)
    {
      m_from_grids_value[tp]=std::min(v, m_from_grids_value[tp]);
    }
    ...
    value(void) const
    {
      return std::min(m_point_value,
                      std::min(m_from_grids_value[0],
                               m_from_grids_value[1]));
    }

    float
    fixed_grid_value(enum coordinate_type tp) const
    {
      return m_from_grids_value[tp];
    }

  private:
    float m_point_value;
    vec2 m_from_grids_value;
  };
  ...
  public:
    ...
    compute_distance_values(boost::multi_array<distance_return_type, 2> &victim, 
                            float max_dist_value, const ivec2 &fudge_offset);
                          ...
                          float max_dist_value, const ivec2 &fudge_offset)  
  {
    ...
    compute_outline_point_values(victim);
    compute_zero_derivative_values(victim);
    compute_fixed_line_values(victim);
  }

  void
  OutlineData::
  init_distance_values(boost::multi_array<distance_return_type, 2> &victim, float max_dist_value)
  {
    //init victim:
    for(int x=0;x<m_bitmap_size.x();++x)
      {
        for(int y=0;y<m_bitmap_size.y();++y)
          {
            victim[x][y].m_distance.init(2.0f*max_dist_value);
          }
      }
  }
            ...
            x<end_x; ++x)
          {
            for(int y=std::max(0, ipt.y()-2),
                ...
                y<end_y;++y)
              {
                ...
                victim[x][y].m_distance.update_point_value(dc);
              }
          }
      }
                 ...
                 for(int cindex=std::max(0, prev_index-1),
                     ...
                     cindex<last_index; ++cindex)
                   {
                     ...
                     victim[pixel.x()][pixel.y()].
                       m_distance.update_fixed_grid_value(dc, coord_tp);
  ...
  if(m_resize_required)
    {
      ...
      */
      ...
      if(m_uses_mipmapping and false)
        {
...
generate_character(uint32_t glyph_index)
{
  ...
  boost::multi_array<distance_return_type, 2> distance_values(boost::extents[bitmap_sz.x()][bitmap_sz.y()]);
  outline_data.compute_distance_values(distance_values, max_dist, ivec2(0,0));
...
#ifndef USE4CHANNEL_FONT_TEXTURE
          float v0;
          v0=std::min(distance_values[xx][yy].m_distance.value()/max_dist, 1.0f);
          image_buffer[location].x()=pixel_value_from_distance(v0, outside);
#else
          float v0, v1, v2;

          v0=std::min(distance_values[xx][yy].m_distance.point_value()/max_dist, 1.0f);
          v1=std::min(distance_values[xx][yy].m_distance.fixed_grid_value(x_fixed)/max_dist, 1.0f);
          v2=std::min(distance_values[xx][yy].m_distance.fixed_grid_value(y_fixed)/max_dist, 1.0f);

          image_buffer[location].x()=pixel_value_from_distance(v0, outside);
          image_buffer[location].y()=pixel_value_from_distance(v1, outside);
          image_buffer[location].z()=pixel_value_from_distance(v2, outside);
#endif
              ...
              for(int xx=0; xx<m_ttf_face->glyph->bitmap.width and xx<glyph.m_mipmaps[m].m_texture_size.x(); ++xx)
                {
                  ...
                  for(int coord=0; coord<pixel_type::array_size-1; ++coord)
                    {
                      GLuint v;
                      v=glyph.m_mipmaps[m-1].m_pixels[loc0][coord];
                      v+=glyph.m_mipmaps[m-1].m_pixels[loc1][coord];
                      v+=glyph.m_mipmaps[m-1].m_pixels[loc2][coord];
                      v+=glyph.m_mipmaps[m-1].m_pixels[loc3][coord];
                      v=std::min( GLuint(255), v/4);
                      mip_image_buffer[location][coord]=v; 
                    }
                }
            }
