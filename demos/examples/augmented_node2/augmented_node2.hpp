/*! 
 * \file augmented_node2.hpp
 * \brief file augmented_node2.hpp
 * 
...
public:
  ...
  extract_values(reorder_c_array<float> out_values)
  {
    ...
    out_values[BaseNode::number_per_node_values + 0] = m_inner_radius;
    out_values[BaseNode::number_per_node_values + 1] = m_outer_radius;
  }

  //the actual radii values, we make them public.
  float m_inner_radius, m_outer_radius;

  //we can also attach data to a node that is NOT transmitted to GL, like a velocity vector:
  vec2 m_position_velocity;
  float m_ID;
};
