...
/*!\details
  In this example we demonstrate clip-out
  of \ref WRATHLayer and how to use clip-out
  to select which items are affected by
  a clip-out item
 */
...
public:
  ...
  command_line_argument_value<int> m_num_sides_on_clip_widgets;
  command_line_argument_value<int> m_num_widgets;

  cmd_line_type(void):
    m_num_clip_widgets(3, "num_clip_widgets", "number of clip widgets", *this),
    m_num_sides_on_clip_widgets(40, "num_sides_clip_widget", "number of sides each clip widget has", *this),
            ...
            WRATHImage *image)
{
  ...
  sp=&hoard.fetch(brush, WRATHBaseSource::mediump_precision);
  ...
  hoard.add_state(brush, 
                  params.m_drawer.m_draw_passes[0].m_draw_state);
...
{
public:
  ClipWidget(vec2 pos, int num_sides,
  ...
  {
    m_clip_out_widget=make_widget<PlainWidget>(layer, num_sides,
                                               shader, 
                                               WRATHDrawType(0, WRATHDrawType::clip_outside_draw),
                                               NULL);

    m_clip_in_widget=make_widget<PlainWidget>(child_layer, num_sides,
                                              shader, 
                                              WRATHDrawType(0, WRATHDrawType::clip_inside_draw),
                                              NULL);

    m_ring_widget=make_widget<ColorWidget>(layer, num_sides,
                                           shader, 
...
private:  
  ...
  typedef WRATHLayerRotateTranslateFamilySet::SimpleXSimpleYImageFamily::RectWidget BackgroundWidget;
