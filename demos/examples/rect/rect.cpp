...
/*!\details
  The example shows creating and drawing
  of rectangles with images and/or gradients
  applied.

  Exercises:
   - Why do the widgets made last appear below the widgets made first?
     How would you make the widgets made last appear above WITHOUT
     changing their constructor order?
   - Augment the example to use several different images
   - Experiment with different image node types to make the
     image drawn repeated across the squares
   - USe repeat gradiant node types to make the gradient repeat too.
*/
...
private:
  ...
  //augment the node type with a velocity vector and a size
  class NodeWithVelocity:public Node
  {
  public:
    NodeWithVelocity(Node *p):
      Node(p),
      m_velocity(0.0f, 0.0f)
    {}

    NodeWithVelocity(const WRATHTripleBufferEnabler::handle &tr):
      Node(tr),
      m_velocity(0.0f, 0.0f)
    {}
  ...
  typedef WRATHLayerItemWidget<NodeWithVelocity>::FamilySet FamilySet; 
  ...
  typedef FamilySet::SimpleXSimpleYImageFamily ImageFamily;
  typedef FamilySet::RadialGradientSimpleXSimpleYImageFamily RadialGradientImageFamily;
  ...
  make_widget(WRATHGradient *grad, WRATHImage *image)
  {
    ...
    //create the widget
    return_value=WRATHNew T(m_layer, brush);
  ...
  void
  move_node(NodeWithVelocity *pnode, float delta_t);
